...
const sendThreeDayEmails = require("../newsletter/threeday")
const sendWeeklyEmails = require("../newsletter/weekly")
...
const weeklyJob = cron.schedule('0 9 * * 0', (_,{users}) => {
  console.log('Running weekly email task');
      sendWeeklyEmails(users).then(()=>{
...
const threeDayJob = cron.schedule('0 10 */3 * *', (_,{users}) => {
  console.log('Running weekly email task');
  //  prisma.user.findMany({where:{emailFrequency:{
  //       lte:3
  //   }}}).then(users=>{
      sendThreeDayEmails(users).then(()=>{
        console.log("Successful weekly email task")
      }).catch(err=>{
        console.log("Unsuccessful Weekly email")
        console.log(err.message)
...
module.exports = {weeklyJob,threeDayJob}